...
- **NCCL is wired** (off by default — heaviest path): `nccl.enabled` stamps `nccl-launcher`/`nccl-peer` (multi-rail, [nccl-pods.yaml](roce-perf/templates/nccl-pods.yaml)); `run_suite.sh --nccl` first **deletes the perftest pods** (they hold the GPUs/rails; hostPath results survive), applies **per-rail source routing** on both pods ([rail_routes.sh](roce-perf/files/rail_routes.sh) — multi-VF pods have overlapping supernet routes, so each rail gets its own table + `ip rule from <rail-ip>` + loose rp_filter) with a same-rail ping check, injects passwordless SSH, builds nccl-tests on both pods, and runs [nccl_one_vs_many.sh](roce-perf/files/nccl_one_vs_many.sh) (`mpirun` pinned to eth0 for its TCP bootstrap — `pml ob1`/`btl tcp` + `if_include` — one-HCA vs all **for each collective in `nccl.collectives`** (all_reduce/alltoall/reduce_scatter/sendrecv — each a distinct traffic pattern; results land in `nccl/<collective>/{one,all}_hca.txt` + a combined `nccl/nccl.json`), RoCE `-x` tuning incl. sysfs-detected `NCCL_IB_GID_INDEX`, preferring the **IPv4-mapped** RoCE v2 GID, typically index 3 — link-local v2 GIDs don't route across the /31 leaf gateways). The ping check also **gates the HCA list**: run_suite passes only the rails that routed as `NCCL_HCA_ONE/ALL` (a QP on a non-routing rail fails `ibv_modify_qp` with errno 19), and the script's own auto-detect maps rail **netdevs**→RDMA devices rather than trusting `/sys/class/infiniband` (privileged pods see host devices there). The NCCL pods also mount a **memory-backed `/dev/shm`** (`nccl.shm.size`, default 8Gi) — the k8s default 64Mi dies with "No space left on device"; `nccl.shm.disable` sets `NCCL_SHM_DISABLE` as an escape hatch.
