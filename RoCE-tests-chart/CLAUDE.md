...
- **NCCL is wired** (off by default — heaviest path): `nccl.enabled` stamps `nccl-launcher`/`nccl-peer` (multi-rail, [nccl-pods.yaml](roce-perf/templates/nccl-pods.yaml)); `run_suite.sh --nccl` injects passwordless SSH, builds nccl-tests on both pods, and runs [nccl_one_vs_many.sh](roce-perf/files/nccl_one_vs_many.sh) (`mpirun` one-HCA vs all, RoCE `-x` tuning incl. sysfs-detected `NCCL_IB_GID_INDEX`).
